...
public class NewUserService
{
    ...
    public async Task<User> ReplaceUserAsync(Guid id, User newUser)
    {
        try
        {
            var userToUpdate = await _context.FindByIdAsync<User>(id) ?? throw new NotFoundException("User not found");
            ...
            var response = await _context.UpdateAsync(userToUpdate.Id, newUser) ?? throw new ConflictException("Updating user failed");
    ...
    public async Task<User> GetUserByIdAsync(Guid userId)
    {
        try
        {
            var response = await _context.FindByIdAsync<User>(userId) ??  throw new NotFoundException("User not found");
    ...
    public async Task DeleteUserAsync(Guid userId)
    {
        try
        {
            ...
            {
                var responseTrip = await _context.FindAndUpdateByPropertyAsync<Trip>(trip.Id, "CreatedBy", sentielId) ?? throw new ConflictException("Trip not updated");
                sentiel.Trips!.Add(responseTrip.Id);
            }

            var responseUser = await _context.FindAndUpdateByPropertyAsync<User>(sentielId, "Trips", trips) ?? throw new ConflictException("Sentiel not updated");
    ...
    public async Task<User> UpdateUserAsync(Guid userId, string property, object value)
    {
        try
        {
            ...
            var response = await _context.FindAndUpdateByPropertyAsync<User>(userId, property, value) ?? throw new NotFoundException("User not found");
            _logger.LogInformation($"User {response.Username} updated");
            return response;
        }
        catch (Exception ex)
    ...
    private async Task<User> CreateSentielIfNotExistsAsync()
    {
        try
        {
            ...
            var response = await _context.CreateAsync(sentinel);
